...
class MongoTypingTestRepository(dbInfos: DatabaseInfos) extends TypingTestRepository:
  ...
  private def toDocument(test: PersistedTypingTest): Document =
    ...
    val completedInfoDoc = new Document()
      ...
      .append("completedDateTime", testData.info.completedDateTime.map(_.toString).orNull)
  ...
  private def fromDocument(doc: Document): PersistedTypingTest =
    ...
    val completedDateTime = Option(completedInfoDoc.getString("completedDateTime"))
      .map(DateTime.parse)
    ...
    val modifiers = testDataDoc.getList("modifiers", classOf[String]).asScala.toSeq
