...
class TestProfile extends AnyFunSuite, Matchers:
  ...
  def validSettings = Set("dark_mode", "notifications")
  ...
  test("UserProfile con dati validi") {
    val profile = UserProfile(None, validName, validEmail, validPassword, validSettings)
  ...
  test("UserProfile con nome vuoto") {
    val profile = UserProfile(None, "", validEmail, validPassword, validSettings)
